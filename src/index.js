...
async function showB2bTariffSelector(botToken, chatId, env) {
  ...
  await sendMessage(botToken, chatId,
    ...
    `━━━━━━━━━━━━━━━━━━━━━\n` +
    `📲 *Где размещать вашу ссылку:*\n\n` +
    `📸 *Instagram* — добавьте в шапку профиля:\n` +
    `_"Примерь причёску до записи →"_ + ваша ссылка\n\n` +
    `🖨 *Распечатать QR-код* — повесить на зеркало, стойку или раздать визитки:\n` +
    `_Клиент сканирует — сразу открывается бот_\n\n` +
    `💬 *WhatsApp* — отправить постоянным клиентам вручную:\n` +
    `_"Попробуй наш новый ИИ-подбор причёски" + ссылка_\n\n` +
    `📢 *WhatsApp рассылка* — по всей базе одним сообщением:\n` +
    `_Отправляете раз в месяц — получаете волну новых записей_\n\n` +
    `━━━━━━━━━━━━━━━━━━━━━\n` +
    ...
    await sendMessage(botToken, userId,
      ...
      (clientLink
        ? `🔗 *Ваша ссылка для клиентов:*\n\`${clientLink}\`\n\n` +
          `Разместите её везде где вас находят клиенты:\n\n` +
          `📸 *Instagram* — вставьте в шапку профиля:\n` +
          `_"Примерь причёску до записи →"_ + ссылка\n\n` +
          `🖨 *QR-код* — распечатайте и повесьте на зеркало или стойку ресепшена.\n` +
          `QR приходит следующим сообщением 👇\n\n` +
          `💬 *WhatsApp* — отправьте постоянным клиентам:\n` +
          `_"Привет! Теперь можно примерить причёску через бота до визита — попробуй!"_ + ссылка\n\n` +
          `📢 *WhatsApp рассылка* — отправьте по всей базе одним сообщением.\n` +
          `_Это самый быстрый способ получить первые записи сегодня._`
        : `Ссылка для клиентов появится в разделе *📊 Мой бот*.`),
      ownerMenuKeyboard()
    );
    if (clientLink) {
...
async function sendPhotoB64(botToken, chatId, b64, caption = '') {
  ...
  if (!res.ok) console.error(`sendPhotoB64 error: ${res.status} ${await res.text()}`);
}
